...
export function persist(store) {
  return () => {
    if (window) {
      ...
      let timestamp = storage.getItem(`fes-timestamp`)
      let now = Number(new Date)
      let raw = store.getState()
      let state = fromJS(raw).filter(whitelisted).toJS()

      if (!timestamp) {
        storage.setItem(`fes-timestamp`, now)
        storage.setItem(`fes-memory`, JSON.stringify(state))
      } else {
        let seconds = 5 * 1000
...
export function rehydrate(initializer, seedState) {
  if (window) {
    let storage = window.localStorage
    let persistedState = storage.getItem(`fes-memory`)
